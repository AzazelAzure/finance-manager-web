...
export function TagPie({ transactions, baseCurrency, isLoading, isError, onRetry, onSelectTag }: Props): ReactNode {
  ...
  return (
    <ChartFrame
      title={tr("dashboard.chart.spendingByTag", locale)}
      ariaLabel={tr("dashboard.chart.spendingByTag.aria", locale)}
      isLoading={isLoading}
      isError={isError}
      onRetry={onRetry}
      isEmpty={empty}
      emptyDescription={tr("dashboard.chart.noTaggedExpenses", locale)}
    >
